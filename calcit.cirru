      ...
      :defs $ {}
              ...
              div
                ...
                div
                  ...
                  comp-keyboard (>> states :keyboard)
                    fn (input d!)
                      case-default input
                        d! cursor $ update state :inputs $ fn (inputs)
                          conj inputs $ join-str input |
                        nil $ raise "|Unknown input"
                        -1 $ d! cursor $ update state :inputs
                          fn (inputs)
                            or (butlast inputs) ([])
                            ...
                            :on-click $ fn (e d!)
                              case-default key (println |TODO)
                                "|⌫" $ if
                                  ...
                                  on-input -1 d!
                                  d! cursor $ update state :buffer $ fn (x)
                                    or (butlast x) ([])
        ...
        'markup-tone $ %{} 'CodeEntry (:doc |)
          :code $ quote $ defn markup-tone (x)
            let
                x1 $ .!replace (unsafe-coerce x 'JsObject)
                  new js/RegExp "|([aoeiuü])(ng?)([→↗↺↘])$"
                  fn (a b b2 c & args)
                    str (mark-vowel b c) b2
                x2 $ .!replace (unsafe-coerce x1 'JsObject)
                  new js/RegExp "|([aoeiuü])([aoeiuü])([→↗↺↘])$"
                  fn (a b b2 c & args)
                    str (mark-vowel b c) b2
                x3 $ .!replace (unsafe-coerce x2 'JsObject)
                  new js/RegExp "|([aoeiuü])r([→↗↺↘])$"
                  fn (a b c & args)
                    str (mark-vowel b c) |r
              .!replace (unsafe-coerce x3 'JsObject)
                new js/RegExp "|([aoeiuü])([→↗↺↘])$"
